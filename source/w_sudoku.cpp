...
w_Sudoku_cell::w_Sudoku_cell(Sudoku& t_s, int t_cnt, w_Sudoku& t_w_Sudoku,
    ...
    if (s(cnt).val == 0) {
        ...
        setFocusPolicy(Qt::NoFocus);    // pre-filled cells cannot be selected for user
...
void w_Sudoku_cell::enterEvent(QEvent* e) {
    ...
    for (int j = 0; j < s.region_size; ++j) {
        w_s.dyn_cell_prop[s.block_to_cnt(curr_block, j)].cell_marked_as_part_of_region =
            true;
    }
...
void w_Sudoku_cell::keyPressEvent(QKeyEvent* e) {
    ...
    switch (e->key()) {
        case Qt::Key_0: emit value_changed_by_user(cnt, 0); break;
        case Qt::Key_1: emit value_changed_by_user(cnt, 1); break;
        case Qt::Key_2: emit value_changed_by_user(cnt, 2); break;
        case Qt::Key_3: emit value_changed_by_user(cnt, 3); break;
        case Qt::Key_4: emit value_changed_by_user(cnt, 4); break;
        case Qt::Key_5: emit value_changed_by_user(cnt, 5); break;
        case Qt::Key_6: emit value_changed_by_user(cnt, 6); break;
        case Qt::Key_7: emit value_changed_by_user(cnt, 7); break;
        case Qt::Key_8: emit value_changed_by_user(cnt, 8); break;
        case Qt::Key_9: emit value_changed_by_user(cnt, 9); break;
        default: QWidget::keyPressEvent(e);    // pass-on to base class
    }
...
w_Sudoku::w_Sudoku(Sudoku& t_s, QPlainTextEdit* t_textConsole, QWidget* parent) :
    ...
    // in Sudoku_view
    connect(this, SIGNAL(update_parent()), parent, SLOT(on_update_requested_by_child()));
...
void w_Sudoku::store_sudoku_for_undo(Sudoku sh) { undo.push_back(sh); }
