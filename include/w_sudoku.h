...
struct w_Sudoku_dyn_cell_properties {  // dynamic = linked to enter & leave events in cell

  bool cell_marked_as_selected{false};        // mark background in tranparend red
  bool cell_marked_as_part_of_region{false};  // mark backgound in transparent blue
  // bool cell_has_input_focus{false};              // if true cell accepts user input
};
...
struct w_Sudoku_cell_properties {  // depend only on status of Sudoku
  ...
  // bool cell_marked_as_hidden_single_region{false};  // mark if part of hidden single
  // region
};
...
struct w_Sudoku_properties {
  ...
  bool show_affected_regions{false};  // show cells affected by current active cell
  bool show_block_background{true};   // show gray blackground in cells of uneven blocks
...
public:
  w_Sudoku_cell(Sudoku& t_s, int t_cnt, w_Sudoku& t_w_Sudoku, QWidget* parent = 0);
